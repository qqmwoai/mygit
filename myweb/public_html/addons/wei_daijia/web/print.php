<?php
       ...
       if($op == 'print_post') {
		...
		if(checksubmit('submit')) {
			...
			$data['sid'] = $sid;
			if(!empty($item) && $id) {
				pdo_update('daijia_print', $data, array('uniacid' => $_W['uniacid'], 'id' => $id));
			} else {
				pdo_insert('daijia_print', $data);
			}
			message('更新打印机设置成功', $this->createWebUrl('print', array('op' => 'print_list')), 'success'); 
		}
			
		} elseif($op == 'print_list') {
			$data = pdo_fetchall('SELECT * FROM ' . tablename('daijia_print') . ' WHERE uniacid = :uniacid ', array(':uniacid' => $_W['uniacid']));
			...
			$psize = 20;
			$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('daijia_order_print') . ' AS a ' . $condition, $params);
			$data = pdo_fetchall('SELECT a.*,b.* FROM ' . tablename('daijia_order_print') . ' AS a LEFT JOIN' . tablename('daijia_orders') . ' AS b ON a.oid = b.id' . $condition . ' ORDER BY addtime DESC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize, $params);
