...
class Beauty_zhongchouModuleSite extends WeModuleSite {
	...
	public function doMobileIndex() {
		...
		$rule = $data['content'];
		$myprofile = pdo_fetch("SELECT * FROM " . tablename('beatuty_zhongchou_member') . " WHERE uniacid ='{$_W['uniacid']}' and openid = '{$_W['openid']}'");
		if($data['share_imagestatus']==1){
			$share_image = $myprofile['avatar'];
		}else{
