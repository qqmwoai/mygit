...
// 粉丝入口
	public function doMobileFansIndex(){
		...
		if($theone['promotertimes'] == 0 && $profile['flag'] == 0){
			$isorder = pdo_fetch('SELECT * FROM '.tablename('eso_sale_order')." WHERE status!= '0' AND  uniacid = :uniacid  AND from_user = :from_user" , array(':uniacid' => $_W['uniacid'],':from_user' => $from_user));
			if(!$isorder){
	...
	public function doMobileCommission(){
		...
		if($op=='display'){
			...
			$list1 = pdo_fetchall("select * from ".tablename('eso_sale_member'). " where uniacid = '{$_W['uniacid']}' and shareid = '{$id}'");
			$money1 = 0;
			$money2 = 0;
			$money3 = 0;
			foreach ($list1 as $key1 => $value1) {
				$commissioningpe1 = pdo_fetchcolumn("SELECT sum((g.commission*g.total)) FROM " .tablename('eso_sale_order')." as o left join ".tablename('eso_sale_order_goods')." as g on o.id = g.orderid and o.uniacid = g.uniacid WHERE o.shareid = ".$list1[$key1]['shareid']." and o.uniacid = ".$_W['uniacid']." and (g.status = 0 or g.status = 1) and o.status >= 3 and o.from_user != '".$from_user."' and  g.createtime>=".$userx['flagtime']);
				$er_list = pdo_fetchall("select * from ".tablename('eso_sale_member'). " where uniacid = '{$_W['uniacid']}' and shareid = ".$list1[$key1]['id']);

				for ($i=0; $i < count($er_list); $i++) { 
				 	$commissioningpe2 = pdo_fetchcolumn("SELECT sum((g.commission2*g.total)) FROM " .tablename('eso_sale_order')." as o left join ".tablename('eso_sale_order_goods')." as g on o.id = g.orderid and o.uniacid = g.uniacid WHERE o.shareid = ".$er_list[$i]['shareid']." and o.uniacid = ".$_W['uniacid']." and (g.status = 0 or g.status = 1) and o.status >= 3 and o.from_user != '".$from_user."' and  g.createtime>=".$userx['flagtime']);
				 	$san_list = pdo_fetchall("select * from ".tablename('eso_sale_member'). " where uniacid = '{$_W['uniacid']}' and shareid = ".$er_list[$i]['id']);

			 		foreach ($san_list as $key3 => $value3) {
			 			$commissioningpe3 = pdo_fetchcolumn("SELECT sum((g.commission3*g.total)) FROM " .tablename('eso_sale_order')." as o left join ".tablename('eso_sale_order_goods')." as g on o.id = g.orderid and o.uniacid = g.uniacid WHERE o.shareid = ".$san_list[$key3]['shareid']." and o.uniacid = ".$_W['uniacid']." and (g.status = 0 or g.status = 1) and o.status >= 3 and o.from_user != '".$from_user."' and  g.createtime>=".$userx['flagtime']);
			 			$money3+=$commissioningpe3;
			 		}
			 		$money2+=$commissioningpe2;
				}
				$money1+=$commissioningpe1;
			}
			$commissioningpe = $money1 + $money2 + $money3;
...
//-----------------------------------web端
	...
	public function doWebOutCommission(){
		...
		$objPHPExcel->getProperties()->setCreator("返利商城")
			->setLastModifiedBy("返利商城")
	...
	public function doMobilelist() {
		...
		if ($re['code'] == 200) {
			...
			if ($content['subscribe'] == 1) { //此人已关注
				...
				header("Location:".$share_data['ydyy']); 
			}
		}
		}
...
*/
	...
	public function doWebCharge() {
		...
		if($op=='list'){
			if($_GPC['submit'] == '搜索'){
				if ($_GPC['mobile']) {
					$list = pdo_fetchall("SELECT * FROM ".tablename('mc_members')."  WHERE uniacid='{$_W['uniacid']}' AND mobile = '{$_GPC['mobile']}' LIMIT" . ($pindex - 1) * $psize . "," . $psize);
					$total = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('mc_members')." WHERE `uniacid` = :uniacid AND mobile = '{$_GPC['mobile']}'", array(':uniacid' => $_W['uniacid']));
				}else{
					$list = pdo_fetchall("SELECT * FROM ".tablename('mc_members')."  WHERE uniacid=".$_W['uniacid']." LIMIT" . ($pindex - 1) * $psize . "," . $psize);
					$total = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('mc_members')." WHERE `uniacid` = :uniacid", array(':uniacid' => $_W['uniacid']));
				}
				$pager = pagination($total, $pindex, $psize);
			...
			if(intval($_GPC['so']) == 1) {
				$total = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('mc_members')." WHERE uniacid = :uniacid", array(':uniacid' => $_W['uniacid']));
				$pager = pagination($total, $pindex, $psize);
				$list = pdo_fetchall("SELECT * FROM ".tablename('mc_members')."  WHERE uniacid=".$_W['uniacid']." ORDER BY uid DESC LIMIT " . ($pindex - 1) * $psize . "," . $psize);
			} else {
				$total = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('mc_members')." WHERE `uniacid` = :uniacid", array(':uniacid' => $_W['uniacid']));
				$pager = pagination($total, $pindex, $psize);
				$list = pdo_fetchall("SELECT * FROM ".tablename('mc_members')." WHERE uniacid=".$_W['uniacid']." ORDER BY uid DESC LIMIT " . ($pindex - 1) * $psize . "," . $psize);
