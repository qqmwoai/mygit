...
if (!class_exists('CommissionModel')) {
	class CommissionModel extends PluginModel {
		...
		function getShop($m) {
			...
			if (empty($shop)) {
				$shop = array('name' => $member['nickname'] . '的小店', 'logo' => $member['avatar'], 'img' => tomedia($set['img']), 'desc' => empty($set['description'])?$set['name']:$set['description'],);
			} else {
				if (empty($shop['name'])) {
				...
				if (empty($shop['desc'])) {
					$shop['desc'] = empty($set['description'])?$set['name']:$set['description'];
				} 
			} 
			return $shop;
		} 
