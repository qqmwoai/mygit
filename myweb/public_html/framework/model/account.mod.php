...
function uni_fetch($uniacid = 0) {
	...
	$owneruid = pdo_fetchcolumn("SELECT uid FROM ".tablename('uni_account_users')." WHERE uniacid = :uniacid AND role = 'owner'", array(':uniacid' => $account));
	load()->model('user');
	...
	$account['grouplevel'] = pdo_fetchcolumn('SELECT grouplevel FROM ' . tablename('uni_settings') . ' WHERE uniacid = :uniacid', array(':uniacid' => $account));
...
function uni_user_module_permission_check($action = '', $in_html = true, $module_name = '') {
	global $_GPC;
	$status = uni_user_permission_exist();
	if(!is_error($status)) {
		return true;
	...
	if(empty($permission) || ($permission[0] != 'all' && !empty($action) && !in_array($action, $permission))) {
		...
		message('您没有对该模块进行操作的权限', referer(), 'error');
	}
	return true;
}
